...
class WaveBoxFrame extends Frame implements ComponentListener, ActionListener,
	...
	void computeFunction() {
		...
		for (i = 0; i != gridSizeX; i++)
			for (j = 0; j != gridSizeY; j++) {
				...
				double mint = max(min(tx1, tx2), max(min(ty1, ty2), min(tz1,
						tz2))) + .001;
				double maxt = min(max(tx1, tx2), min(max(ty1, ty2), max(tz1,
						tz2))) - .001;
				if (maxt < mint) {
					// doesn't hit box
	...
	public void itemStateChanged(ItemEvent e) {
		if (e.getItemSelectable() == setupChooser) {
			...
			setup.deselect();
			setup = (Setup) setupList
					.elementAt(setupChooser.getSelectedIndex());
			hideBars();
			setup.select();
			setup.precompute();
			validate();
		...
		void precompute() {
			...
			for (x = 0; x != maxTerms; x++)
				for (y = 0; y != maxTerms; y++) {
					double xi = x - mxhalf;
					int yi = y - mxhalf;
					dataxy[x][y] = (int) (distmult
							* java.lang.Math.sqrt(xi * xi + yi * yi) + .5);
			...
			for (x = 0; x != maxTerms; x++)
				for (y = 0; y <= maxdist; y++) {
					int xi = x - mxhalf;
					double r = java.lang.Math.sqrt(y * y
							/ (distmult * distmult) + xi * xi)
							* resadj + .00000001;
	...
	class TwoSourcesSetup extends Setup {
		...
		double computePoint(int x, int y, int z) {
			...
			return w1mult * (datadzr[d1][z] * cost1 - datadzi[d1][z] * sint1)
					+ w2mult
					* (datadzr[d2][z] * cost2 - datadzi[d2][z] * sint2);
		}
	...
	class LinearQuadrupoleSetup extends Setup {
		...
		void precompute() {
			...
			for (x = 0; x != maxTerms; x++) {
				...
				for (y = 0; y != maxTerms; y++) {
					double yi = y - mxhalf + .001;
					dataxy1[x][y] = (int) (distmult
							* java.lang.Math.sqrt(xi1 * xi1 + yi * yi) + .5);
					dataxy2[x][y] = (int) (distmult
							* java.lang.Math.sqrt(xi2 * xi2 + yi * yi) + .5);
	...
	class TwoPinholesSetup extends Setup {
		...
		void precompute() {
			...
			for (x = 0; x != maxTerms; x++) {
				...
				for (y = 0; y != maxTerms; y++) {
					dataxy[x][y] = (int) (distmult
							* java.lang.Math.sqrt(xi * xi + y * y) + .5);
					if (dataxy[x][y] > maxdist)
						maxdist = dataxy[x][y];
				}
			}
		...
		void precompute() {
			...
			for (x = 0; x != maxTerms; x++) {
				...
				for (y = 0; y != maxTerms; y++) {
					...
					datar[x][y] = m
							* (bessj0(r1 * mult) + bessj0(r2 * mult) + bessj0(r3
									* mult));
					datai[x][y] = -m
							* (bessy0(r1 * mult) + bessy0(r2 * mult) + bessy0(r3
									* mult));
				}
			}
		}
	...
	class PlaneWaveSetup extends Setup {
		...
		double computePoint(int x, int y, int z) {
			return .05 * (java.lang.Math.cos(x * mult) * cost1 + java.lang.Math
					.sin(x * mult)
					* sint1);
		}
	...
	class TwoPlaneWavesSetup extends Setup {
		...
		double computePoint(int x, int y, int z) {
			...
			return w1mult
					* (java.lang.Math.cos(k1) * cost1 + java.lang.Math.sin(k1)
							* sint1)
					+ w2mult
					* (java.lang.Math.cos(k2) * cost2 + java.lang.Math.sin(k2)
							* sint2);
		}
	...
	double bessj0(double x) {
		...
		if (x < 8.0) {
			...
			ans = java.lang.Math.sqrt(0.636619772 / ax)
					* (java.lang.Math.cos(xx) * ans1 - z
							* java.lang.Math.sin(xx) * ans2);
		}
		return ans;
	}
	...
	double bessy0(double x) {
		...
		if (x < 8.0) {
			...
			ans2 = 40076544269.0
					...
					* (745249964.8 + y
							* (7189466.438 + y
									* (47447.26470 + y
											* (226.1030244 + y * 1.0))));
			ans = (ans1 / ans2) + 0.636619772 * bessj0(x)
			...
			ans = java.lang.Math.sqrt(0.636619772 / x)
					* (java.lang.Math.sin(xx) * ans1 + z
							* java.lang.Math.cos(xx) * ans2);
		}
		return ans;
	}
