...
class RippleFrame extends Frame implements ComponentListener, ActionListener,
	...
	public void init() {
		...
		add(forceBar = new Scrollbar(Scrollbar.HORIZONTAL, forceBarValue = 15,
				1, 1, 30));
		forceBar.addAdjustmentListener(this);

		add(new Label("Brightness", Label.CENTER));
		add(brightnessBar = new Scrollbar(Scrollbar.HORIZONTAL, 27, 1, 1, 1200));
		brightnessBar.addAdjustmentListener(this);

		add(auxLabel = new Label("", Label.CENTER));
		add(auxBar = new Scrollbar(Scrollbar.HORIZONTAL, 1, 1, 1, 30));
			...
			for (iter = 0; iter != iterCount; iter++) {
				...
				for (j = jstart; j != jend; j += jinc) {
					...
					for (; gi != giEnd; gi += iinc) {
						...
						//mean of neighboor values
						float basis = (nexti + previ + nextj + prevj) * .25f;
						
						if (true) {//exceptional[gi]) {
							//só otimiza performance...
							if (curMedium != medium[gi]) {
								...
								sinth = (float)(Math.sin(tadd2) * dampcoef);
								scaleo = (float)Math.cos(tadd2);
								
							}
							//if this is erased, the walls lose effect
							if (walls[gi])
								continue;
							/*int count = 4;
				...
				t += tadd;
				if (sourceCount > 0) {
					double w = forceBar.getValue() * (t - forceTimeZero)
							* freqMult;
					double w2 = w;
					boolean skip = false;
					switch (auxFunction) {
					...
					case SWF_PULSE: {
						...
						double pulselen2 = forceBar.getValue() * .2;
					...
					sources[0].v = sources[2].v = (float) (v * setup
							.sourceStrength());
					sources[1].v = sources[3].v = (float) (v2 * setup
							.sourceStrength());
					if (sourcePlane) {
						if (!skip) {
...
/*	void filterGrid() {
		...
		if (fixedEndsCheck.getState())
			return;
		if (sourceCount > 0 && forceBarValue > 23)
			return;
	...
	void drawPlaneSource(int x1, int y1, int x2, int y2, float v, double w) {
		 ...
		 * (auxBar.getValue()-15)*3.8*forceBar.getValue()*freqMult;
		 */
	...
	void editFuncPoint(int x, int y) {
		...
		if (modeChooser.getSelectedIndex() == MODE_WALLS) {
			...
			medium[gi] = (dragSet) ? MEDIUM_MAX : 0;
			calcExceptions();
		} else {
			if (!dragSet && !dragClear) {
				dragClear = func[gi] > .1;
	...
	class PhasedArray1Setup extends Setup {
		...
		float calcSourcePhase(double ph, float v, double w) {
			ph *= (auxBar.getValue() - 15) * 3.8 * forceBar.getValue()
					* freqMult;
	...
	class PhasedArray2Setup extends PhasedArray1Setup {
		...
		float calcSourcePhase(double ph, float v, double w) {
			...
			ph *= forceBar.getValue() * freqMult * 108;
			return (float) Math.sin(w + ph);
		}
	...
	class CoatingSetup extends Setup {
		...
		void select() {
			...
			// n = sqrt(v2/c2)
			double nmax = Math.sqrt(1 - MEDIUM_MAX_INDEX);
			double nroot = Math.sqrt(nmax);
			int mm = (int) ((1 - nmax) * MEDIUM_MAX / MEDIUM_MAX_INDEX);
			for (i = 0; i != gridSizeX; i++)
	 ...
	 * fixedEndsCheck.setState(false); setForceBar(3); int i; int cx =
	...
	class PlanarConvexLensSetup extends Setup {
		...
		void select() {
			...
			for (i = 0; i <= x0; i++) {
				...
				for (; y >= 0; y--) {
					setMedium(cx + i, cy + y, MEDIUM_MAX / 2);
					setMedium(cx - i, cy + y, MEDIUM_MAX / 2);
				}
			}
			setForceBar(19);
	...
	class CircularPrismSetup extends Setup {
		...
		void select() {
			...
			for (i = 0; i < x0; i++) {
				...
				for (; y >= 0; y--) {
					setMedium(cx + i, cy + y, MEDIUM_MAX);
					setMedium(cx - i, cy + y, MEDIUM_MAX);
					setMedium(cx + i, cy - y, MEDIUM_MAX);
					setMedium(cx - i, cy - y, MEDIUM_MAX);
				}
	...
	class RightAnglePrismSetup extends Setup {
		...
		void select() {
			...
			for (i = -x0; i < x0; i++)
				for (j = -y0; j <= i; j++)
					setMedium(cx + i, cy + j, MEDIUM_MAX);
			for (i = 0; i != windowWidth; i++)
